...
export function addToHistory({ id, actionType, value, desc, today }: HistoryProperties): void {
...
export function addToBug(properties: BugProperties): Record<string | number, unknown> {
    const bugId = properties.id ?? Object.keys(bugsObject).length + 1;
    const priority =
        properties.priority !== undefined && !Number.isNaN(properties.priority)
            ? properties.priority
            : 0;
