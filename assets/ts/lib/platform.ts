...
export function createNotify(properties: NotificationProperties = {}): void {
    ...
    if (time < 3000 || time > 10_000) time = 3000;
...
export function copyText(text: string): void {
    navigator.clipboard
        ...
        .catch((error: unknown) => console.error("Failed to copy text: ", error));
}

/** Converts emoji in a string to Twemoji SVG markup. */
export function parseTwemojiString(text: string): string {
    return twemoji.parse(text, { folder: "svg", ext: ".svg" });
}

/** Converts emoji contained by an element to Twemoji SVG markup. */
export function parseTwemojiElement(element?: Element | null): void {
    if (element) twemoji.parse(element, { folder: "svg", ext: ".svg" });
}
...
export async function getGithubToken(): Promise<string | false> {
    const localData: unknown = await window.electron.getLocal();
    if (
        localData &&
