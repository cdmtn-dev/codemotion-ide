...
export function formatUnix(timestamp: number, format = "{dd}.{mm}.{yyyy}, {hh}:{ii}:{ss}"): string {
    ...
    if (!format) return `${dd}.${mm}, ${hh}:${ii}:${ss}`;
    return format
        .replaceAll("{dd}", dd)
        .replaceAll("{mm}", mm)
        .replaceAll("{hh}", hh)
...
export function transparentColor(color: string, alpha = 1): string {
