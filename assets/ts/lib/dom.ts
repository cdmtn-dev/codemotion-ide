...
export const tabName = document.querySelector<HTMLElement>("#tab-name");

/** Updates or removes the counter displayed beside the tab title. */
export function setTabNameCounter(count: string | number | false): void {
    if (!tabName) return;
...
export function setTabName(text: string): void {
    if (tabName) tabName.innerHTML = text;
}

/** Shows the status indicator and schedules it to hide after its transition. */
export function showIndicator(time = 1500, callback?: (indicator: Element) => void): void {
...
export const animate = {
    ...
    blurReplace({ add, remove }: { add?: Element; remove?: Element }): void {
...
export function handlePopups(): void {
    ...
    document.addEventListener("click", (event) => {
        ...
        popups.forEach((popup) => {
            ...
            if (popup.contains(target)) clickedInsidePopup = true;
        });
...
export function SmoothScroll(target: Document | HTMLElement, speed: number, smooth: number): void {
    ...
    if (target === document) {
        ...
        scrollTarget = target as HTMLElement;
    }
    let moving = false;
    ...
    const scrolled = (event: Event) => {
        ...
        const wheel = event as WheelEvent & { detail?: number; wheelDelta?: number };
    ...
    const requestFrame =
        ...
        ((callback: FrameRequestCallback) => window.setTimeout(callback, 1000 / 50));
...
export function generateAvatar(name: string): string {
    ...
    const hslToHex = (lightness: number) => {
        const saturation = 0.6;
        const normalizedLightness = lightness / 100;
        const k = (part: number) => (part + hue / 30) % 12;
...
export function handleOnWheelScrollX(): void {
    document
        .querySelectorAll<HTMLElement>(".code-tabs, .commands .commands-suggest")
        .forEach((el) => {
            el.addEventListener(
                "wheel",
                (event) => {
...
export function scrollToBottomSmooth(element: HTMLElement): void {
    element.scrollTo({ top: element.scrollHeight, behavior: "smooth" });
}

/** Lists custom property names declared by accessible `:root` rules. */
export function getAllCSSVariables(): string[] {
    return Array.from(document.styleSheets)
        .filter((sheet) => sheet.href === null || sheet.href.startsWith(window.location.origin))
        .flatMap((sheet) => Array.from(sheet.cssRules))
        .filter(
            (rule): rule is CSSStyleRule => "selectorText" in rule && rule.selectorText === ":root",
        )
...
export function changeTagName(oldElement: Element, newTagName: string): Element {
...
export function fitAceHeight(editor: any, minHeight = 50, maxHeight = 800): void {
    const height = Math.min(
        maxHeight,
        Math.max(minHeight, editor.session.getLength() * editor.renderer.lineHeight),
    );
...
export function initializeInputFocusState(): void {
