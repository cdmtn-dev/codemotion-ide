import { getFileIcon } from "../iconRegistry.js";

export class _Languages {
    static contexts = {};

    static languages = {
        default: {
            name: "Text",
            icon: "document",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        js: {
            name: "JavaScript",
            icon: "js",
            iconExt: "svg",
            mode: "javascript",
            color: "#f7df1e",
        },
        ts: {
            name: "TypeScript",
            icon: "ts",
            iconExt: "svg",
            mode: "typescript",
            color: "#3178c6",
        },
        css: {
            name: "CSS",
            icon: "css",
            iconExt: "svg",
            mode: "css",
            color: "#264de4",
        },
        scss: {
            name: "SCSS",
            icon: "scss",
            iconExt: "svg",
            mode: "sass",
            color: "#cf649a",
        },
        php: {
            name: "PHP",
            icon: "php",
            iconExt: "svg",
            mode: "php",
            color: "#777bb4",
        },
        html: {
            name: "HTML",
            icon: "html",
            iconExt: "svg",
            mode: "html",
            color: "#e34c26",
        },
        md: {
            name: "Markdown document",
            icon: "md",
            iconExt: "svg",
            mode: "markdown",
            color: "#083fa1",
        },
        py: {
            name: "Python",
            icon: "py",
            iconExt: "svg",
            mode: "python",
            color: "#ffd931",
        },
        todo: {
            name: "To-Do List",
            icon: "todo",
            iconExt: "svg",
            mode: "markdown",
            color: "#6c757d",
        },
        gitignore: {
            name: "GIT File",
            icon: "gitignore",
            iconExt: "svg",
            mode: "text",
            color: "#f14e32",
        },
        c: {
            name: "C",
            icon: "c",
            iconExt: "svg",
            mode: "c_cpp",
            color: "#555555",
        },
        cs: {
            name: "C#",
            icon: "csharp",
            iconExt: "svg",
            mode: "csharp",
            color: "#178600",
        },
        cpp: {
            name: "C++",
            icon: "cpp",
            iconExt: "svg",
            mode: "c_cpp",
            color: "#00599c",
        },
        json: {
            name: "JSON",
            icon: "json",
            iconExt: "svg",
            mode: "json",
            color: "#e9a949",
        },
        txt: {
            name: "Text",
            icon: "default",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        rs: {
            name: "Rust",
            icon: "rust",
            iconExt: "svg",
            mode: "rust",
            color: "#dea584",
        },
        mjs: {
            name: "JavaScript Module",
            icon: "js",
            iconExt: "svg",
            mode: "javascript",
            color: "#f7df1e",
        },
        mts: {
            name: "TypeScript Module",
            icon: "ts",
            iconExt: "svg",
            mode: "typescript",
            color: "#3178c6",
        },
        tsbuildinfo: {
            name: "TS Build Info",
            icon: "json",
            iconExt: "svg",
            mode: "json",
            color: "#9aa0a6",
        },
        example: {
            name: "Environment Example",
            icon: "env",
            iconExt: "svg",
            mode: "text",
            color: "#6c757d",
        },
        yml: {
            name: "YAML",
            icon: "yml",
            iconExt: "svg",
            mode: "yaml",
            color: "#cb171e",
        },
        yaml: {
            name: "YAML",
            icon: "yml",
            iconExt: "svg",
            mode: "yaml",
            color: "#cb171e",
        },
        jsx: {
            name: "React JavaScript",
            icon: "react",
            iconExt: "svg",
            mode: "jsx",
            color: "#61dafb",
        },
        tsx: {
            name: "React TypeScript",
            icon: "react",
            iconExt: "svg",
            mode: "typescript",
            color: "#3178c6",
        },
        cjs: {
            name: "JavaScript (CommonJS)",
            icon: "js",
            iconExt: "svg",
            mode: "javascript",
            color: "#f7df1e",
        },
        cts: {
            name: "TypeScript (CommonJS)",
            icon: "ts",
            iconExt: "svg",
            mode: "typescript",
            color: "#3178c6",
        },
        svg: {
            name: "SVG",
            icon: "svg",
            iconExt: "svg",
            mode: "xml",
            color: "#ffb13b",
        },
        env: {
            name: "Environment",
            icon: "env",
            iconExt: "svg",
            mode: "text",
            color: "#6c757d",
        },
        ps1: {
            name: "PowerShell",
            icon: "ps",
            iconExt: "svg",
            mode: "text",
            color: "#012456",
        },
        png: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        jpg: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        jpeg: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        gif: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        webp: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        ico: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        bmp: {
            name: "Image",
            icon: "image",
            iconExt: "svg",
            mode: "text",
            color: "#9aa0a6",
        },
        ttf: {
            name: "Font",
            icon: "font",
            iconExt: "svg",
            mode: "text",
            color: "#d4b483",
        },
        woff: {
            name: "Font",
            icon: "font",
            iconExt: "svg",
            mode: "text",
            color: "#d4b483",
        },
        woff2: {
            name: "Font",
            icon: "font",
            iconExt: "svg",
            mode: "text",
            color: "#d4b483",
        },
        otf: {
            name: "Font",
            icon: "font",
            iconExt: "svg",
            mode: "text",
            color: "#d4b483",
        },
        eot: {
            name: "Font",
            icon: "font",
            iconExt: "svg",
            mode: "text",
            color: "#d4b483",
        },
        lua: {
            name: "Lua",
            icon: "lua",
            iconExt: "svg",
            mode: "lua",
            color: "#1e81bb",
        },
        luau: {
            name: "Luau",
            icon: "lua",
            iconExt: "svg",
            mode: "lua",
            color: "#1e81bb",
        },
        go: {
            name: "GO",
            icon: "go",
            iconExt: "svg",
            mode: "golang",
            color: "#00ACD7",
        },
        bat: {
            name: "BAT File",
            icon: "bat",
            iconExt: "svg",
            mode: "batchfile",
            color: "#444444",
        },
        wasm: {
            name: "WebAssembly",
            icon: "wasm",
            iconExt: "svg",
            mode: "wast",
            color: "#3856ff",
        },
        java: {
            name: "Java",
            icon: "java",
            iconExt: "svg",
            mode: "java",
            color: "#f03b3b",
        },
        kt: {
            name: "Kotlin",
            icon: "kotlin",
            iconExt: "svg",
            mode: "kotlin",
            color: "#cc3bf0",
        },
        vue: {
            name: "Vue.js",
            icon: "vue",
            iconExt: "svg",
            mode: "vue",
            color: "#46cc82",
        },
        exe: {
            name: "Executable file",
            icon: "exe",
            iconExt: "svg",
            mode: "text",
            color: "#a51212",
        },
    };

    static addContext(name, value) {
        _Languages.contexts[name] = value;
    }
    static getContext(name) {
        if (name in _Languages.contexts) {
            return _Languages.contexts[name].value;
        }
        return null;
    }

    static list() {
        return _Languages.languages;
    }

    static update(languageObjectName, object) {
        if (languageObjectName in _Languages.languages) {
            _Languages.languages[languageObjectName] = object;
        }
    }

    static add({ id, name, icon, iconExt, mode, customIcon, customLanguage }) {
        customLanguage = customLanguage == undefined ? false : customLanguage;

        const languageStructure = {
            name,
            icon,
            iconExt,
            mode,
            customLanguage,
        };

        if (customIcon) {
            languageStructure.customIcon = customIcon;
            delete languageStructure.iconExt;
        }

        _Languages.languages[id] = languageStructure;
    }

    static get(name) {
        if (name in _Languages.languages) {
            return _Languages.languages[name];
        }
        return _Languages.languages.default;
    }

    static async getIcon(name) {
        if (!name) return "document.svg";
        const info = _Languages.get(name);
        if (info && info.customIcon) {
            return info.icon;
        }
        return getFileIcon(name);
    }

    static async getIconPath(name) {
        const info = _Languages.get(name);
        const icon = await _Languages.getIcon(name);

        if (info && info.customIcon) {
            return icon;
        }
        return `../assets/media/icons/symbols/files/${icon}`;
    }
}
