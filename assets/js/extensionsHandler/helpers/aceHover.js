...
export function enableAceHover(editor, docs, props) {
    ...
    function showTooltip(x, y, data, key, match) {
        ...
        if (data.type && docTypesArray.includes(data.type)) {
            const type = DocumentationTypes.list().find((item) => item.name === data.type);

            if (type) {
                const tooltipKey = tooltip.querySelector(".ace-documentation__tooltip-key");
                const tooltipType = tooltip.querySelector(".ace-documentation__tooltip-type");
