export class BottomWindow {
    ...
    #init() {
        ...
        document.querySelector(".bottom-window__container").appendChild(win);

        this.win = win;
        this.winContent = win.querySelector(".bottom-window__content");
        this.isFullscreen = false;

        win.querySelector("#bottomWindowClose").addEventListener("click", () => this.hide());

        win.querySelector(".bottom-window__resize-handle").addEventListener(
            "pointerdown",
            (event) => this.#handleResizeStart(event),
        );
    }
