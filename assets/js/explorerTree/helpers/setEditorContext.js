...
export async function setEditorContext(
    properties = {},
    { editor, language, updateEditorData, path, settings },
) {
    ...
    const setScriptContext = async (isTypeScript) => {
        ...
        const getAst = isTypeScript ? window.electron.typescriptAST : window.electron.javascriptAST;
    ...
    if (
        "editor" in settings &&
        "goContextParser" in settings.editor &&
        settings.editor.goContextParser == false
    ) {
