import { handleOnWheelScrollX, type } from "../../assets/js/lib.js";
import { createCommandRegex, parse, trimSpaces } from "./parse.js";
...
const commands = {
    ...
    "-m": (name) => {
        if (name in modules) {
            renderMsg(
                time,
                `${name}@${modules[name].version} Module. ${modules[name].description}\nGet module version: -mv ${name}\nGet module permissions: -mp ${name}`,
            );
            ...
            renderError(
                time,
                `Module "${name}" not defined in this scope. Defined modules: ${Object.keys(modules).join(", ")}`,
            );
        }
    },
    "-mv": (name) => {
    ...
    font: (name) => {
        ...
        if (name.length == 0) {
            renderMsg(
                time,
                `Aviable fonts: ${fonts.join(", ")}. Current: ${localStorage.getItem("font") == null ? "default" : localStorage.getItem("font")}`,
            );
    ...
    fetch: async (url) => {
        if (url.startsWith("http")) {
            ...
            renderError(
                time,
                "The fetch command accepts only URLs. \nExample: <code>fetch https://</code>",
            );
        }
    },
    var: (name) => {
...
function parseCommand(command, silent = false) {
    ...
    Object.keys(variables).forEach((v) => {
        commands[`set:${v}`] = (value) => {
            if (variables[v].type == "const") {
                renderError(
                    time,
                    `Error in the declaration of the variable "${v}": the variable is a constant`,
                );
            } else {
                setVariable({ name: v, value, type: "default" });
            }
        };
    });
    ...
    function executeCommand(command) {
        ...
        const error = `Command "${prefix}" doesn't exists`;
...
window.electron.onDebugData((data) => {
    ...
    const time = `[${formatTimeHtml(data.time)}]`;
...
commandInput.addEventListener("input", (event) => {
    ...
    const finder = findMatches(text, Object.keys(commands));

    const suggest = document.querySelector(".commands-suggest");
    suggest.innerHTML = "";
