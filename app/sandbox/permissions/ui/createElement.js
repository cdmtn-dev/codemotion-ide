        ...
        const properties = {
            ...
            setPosition: (object) => {
                ...
                if (positionsMatch.length > 0) {
                    c.error(
                        `[${type}:setPosition] Undefined position name(-s): ${positionsMatch.join(", ")}`,
                    );
                }
        ...
        if (type == "image") {
            properties["src"] = (srcPath) => {
                ...
                if (!allowedImageFormats.includes(srcBase.split(".").pop())) {
                    c.error(
                        `[${type}:src] This image format is not supported. Supported formats: ${allowedImageFormats.join(", ")}`,
                    );
                }
        ...
        if (type == "topbarItem") {
            properties["setup"] = (properties = {}) => {
                if ("image" in properties) {
                    ...
                    const imgQuery = properties.image.includes("?")
                        ? "?" + properties.image.split("?")[1]
                        : "";
                    if (!allowedImageFormats.includes(imgBase.split(".").pop())) {
                        c.error(
                            `[${type}:setup:image] This image format is not supported. Supported formats: ${allowedImageFormats.join(", ")}`,
                        );
                    }
