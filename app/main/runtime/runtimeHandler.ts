...
import fs from "fs";
import type { RunPythonPayload } from "../payloads";
...
ipcMain.handle(
    "run-python-code",
    (_: IpcMainInvokeEvent, data: RunPythonPayload): Promise<RunPythonResult> => {
        return new Promise((resolve) => {
            ...
            try {
                ...
                if (!py) {
                    return finish({
                        type: "python_not_found",
                        result: useEmbed ? "Embedded Python not found" : "System Python not found",
                    });
                }
