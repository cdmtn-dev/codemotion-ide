...
import { dialog, type IpcMainInvokeEvent, ipcMain, shell } from "electron";
import fs from "fs";
...
import type { SaveContentPayload } from "../payloads";
...
ipcMain.handle(
    "save-file",
    async (_: IpcMainInvokeEvent, fullPath: string, content: string) =>
        await saveFile(fullPath, content),
);
ipcMain.handle(
    "readFileContent",
    async (_: IpcMainInvokeEvent, filePath: string, encoding = "utf8") =>
        readFileContent(filePath, encoding),
);
ipcMain.handle("readDirTree", async (_: IpcMainInvokeEvent, rootPath: string, options = {}) =>
    readDirTree(rootPath, options),
);
...
ipcMain.handle(
    "ask-to-save-content",
    async (_: IpcMainInvokeEvent, payload: SaveContentPayload) => {
        try {
...
ipcMain.handle(
    ...
    async (
        ...
        filePath: string,
        parentPath: string,
    ): Promise<{ success: boolean; result: string | Error }> => {
        try {
